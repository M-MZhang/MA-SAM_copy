    ...
    def predict_masks(
        ...
        for i in range(self.num_layer-1, -1, -1): # use the reversed number to start from the end
            ...
            if i == self.num_layer-1:
                ...
                src = self.fusion_list[i](torch.cat([src, image_embeddings[i]], dim=1)) # c=256*2 -> 256
                src = src.flatten(2).permute(0, 2, 1)
...
class Sam_task(nn.Module):
    ...
    def __init__(
        ...
        # attn_layers = [i for i in range(0, sam_model.image_encoder.global_attn_indexes[0])].extend(sam_model.image_encoder.global_attn_indexes)
